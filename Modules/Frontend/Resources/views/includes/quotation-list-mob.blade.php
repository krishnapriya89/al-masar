        ...
        <div class="accordion-body">
            <div class="dBlock">
                <ul>
                    ...
                    <li><span>Total Price:</span class="quotation-price-{{ $quotation->uid }}">{{ $quotation->priceWithSymbol($quotation->converted_total_bid_price) }}</li>
                    <li><span>Status:</span>
                        <div class="sts {{ $quotation->status_class }} quotation-status-{{ $quotation->uid }}">{{ $quotation->status_value }}</div>
                    </li>
                </ul>
            </div>
                ...
                <div class="accordion-item">
                    <h2 class="accordion-header" id="flush-heading{{ $loop->iteration }}">
                        <div class="submit-quotation-div">
                            @if ($quotation->quotationDetails->whereIn('status', [0, 1])->count() < 1 &&
                                $quotation->acceptedQuotationDetails->count() > 0)
                                <a href="{{ route('checkout', $quotation->uid) }}" class="continue hoveranim"><span>Continue to
                                        Checkout</span></a>
                            @endif
                        </div>
                        ...
                        @foreach ($quotation->quotationDetails as $quotation_detail)
                            <div class="accordion-body">
                                <ul>
                                    ...
                                    <li><span>Price:</span>{{ $quotation_detail->priceWithSymbol($quotation_detail->converted_bid_price) }}</li>
                                    <li><span>Status:</span>
                                        <div class="sts {{ $quotation_detail->status_class }} quotation-detail-status-{{ $quotation_detail->id }}">{{ $quotation_detail->status_value }}</div>
                                    </li>
                                    <li><span>Admin Approved Price:</span>{{ $quotation_detail->admin_approved_price != 0 ? $quotation_detail->priceWithSymbol($quotation_detail->converted_admin_approved_price) : '--' }}</li>
                                    <li><span>Specifications:</span>{{ $quotation_detail->product->specification }}</li>
