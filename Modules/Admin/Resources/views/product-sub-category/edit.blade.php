...
@section('title', 'Update Product Sub Category')
@section('content')
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Update Product Sub Category</h3>
    ...
    <!-- form start -->
    <form method="POST" action="{{ route('product-sub-category.update', $sub_category->id) }}"
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror"
                                           id="title" name="title" value="{{ old('title', $sub_category->title) }}">
                                    @error('title')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control @error('sort_order') is-invalid @enderror"
                                           ...
                                           value="{{ old('sort_order', $sub_category->sort_order) }}">
                                    @error('sort_order')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('status') is-invalid @enderror" id="status"
                                            name="status">
                                        <option
                                            value="1" {{ old('status', $sub_category->status) == '1' ? 'selected' : '' }}>
                                            Active
                                        </option>
                                        <option
                                            value="0" {{ old('status', $sub_category->status) == '0' ? 'selected' : '' }}>
                                            Inactive
                                        </option>
                                    </select>
                                    @error('status')
                            ...
                            <div class="col-md-6 not-last-child-div">
                                <div class="form-group">
                                    ...
                                    <select class="form-control @error('is_last_child') is-invalid @enderror"
                                            ...
                                            name="is_last_child">
                                        <option
                                            value="1" {{ old('is_last_child', $sub_category->is_last_child) == '1' ? 'selected' : '' }}>
                                            Yes
                                        </option>
                                        <option
                                            value="0" {{ old('is_last_child', $sub_category->is_last_child) == '0' ? 'selected' : '' }}>
                                            No
                                        </option>
                                    </select>
                                    @error('is_last_child')
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="main_category" name="main_category"
                                        ...
                                        @foreach($main_categories as $main_category)
                                            <option
                                                {{ old('main_category', $sub_category->product_main_category_id) == $main_category->id ? 'selected' : '' }} value="{{ $main_category->id }}">{{ $main_category->title }}</option>
                                        @endforeach
                                    </select>
                                    @error('main_category')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                ...
                                <div class="form-group">
                                    ...
                                    <select id="sub_category" name="sub_category"
                                        ...
                                        @foreach($sub_categories as $sub_category_single)
                                            <option
                                                {{ old('sub_category', $sub_category->parent_id) == $sub_category_single->id ? 'selected' : '' }} value="{{ $sub_category_single->id }}">{{ $sub_category_single->title }}</option>
                                        @endforeach
                                    </select>
                                    @error('sub_category')
                                    <span class="invalid-feedback">{{ $message }}</span>
...
@push('js')
    <script>
        ...
        $(document).ready(function () {
            ...
            @if(old('sub_category', $sub_category->parent_id))
